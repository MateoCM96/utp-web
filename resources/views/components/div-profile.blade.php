        ...
        <div class="grid sm:grid-cols-1 grid-cols-3 w-full md:w-4/12 lg:w-4/12 items-center">
            ...
            <div class="pl-2 pr-6 text-start jus sm:hidden block col-span-2">
                <p class="text-gray-800 text-xl flex items-center"> {{ $user->username }}
                    <x-icons.verified-icon/>
                </p>
                @auth
                    @if ($user->id === auth()->user()->id)
                        <!-- Si el usuario es el mismo que el que está logueado -->
                        <a class="mt-5 block w-full text-gray-800 text-sm hover:bg-gray-300 bg-gray-200 py-2 px-4 rounded-lg font-semibold cursor-pointer"
            ...
            <div class="p-1">
                <div class="grid grid-cols-2 gap-4 items-center py-3">
                    ...
                    <div class="px-2 text-center sm:text-start hidden sm:block">
                        @auth
                            @if ($user->id === auth()->user()->id)
                                <a class=" text-gray-800 text-sm hover:bg-gray-300 bg-gray-200 py-2 px-4 rounded-lg font-semibold cursor-pointer"
                                    href="/user">Editar perfil</a>
                            @else
                                <button
                ...
                <div class="grid grid-cols-3 gap-4 items-center py-3 px-4 sm:px-0 border-t-2 sm:border-t-0">
                    <x-div-count num="0" text="publicaciones"/>
                    <x-div-count num="0" text="seguidores"/>
                    <x-div-count num="0" text="seguidos"/>
                </div>

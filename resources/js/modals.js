document.addEventListener("DOMContentLoaded", function () {
    ...
    const descriptionTextarea = modalStorePost.querySelector(
        "#descriptionTextarea"
    );
    ...
    function deletePost(postId) {
        ...
        formData.append("_method", "DELETE"); //metodo spoofing
        formData.append("_token", "{{csrf_token()}}"); //metodo spoofing
    ...
    document
        .querySelectorAll(".openModalStorePost")
    ...
    document
        .querySelector(".closeModalStorePost")
    ...
    document
        .querySelector(".closeModalshowPost")
        .addEventListener("click", (event) => {
    ...
    document
        ...
        .addEventListener("click", function (element) {
            ...
            const pickerOptions = {
                ...
                emojiSize: "18",
                categories:
                    "frequent, activity, nature, foods, objects, people, symbols, places",
                onEmojiSelect: (emoji) => {
                    console.log(descriptionTextarea.value.length);
                    descriptionTextarea.value =
                        descriptionTextarea.value + emoji.native;
                    countCharacter();
                    divEmoji.innerHTML = "";
    ...
    document.addEventListener("click", function (event) {
        if (
            event.target !== divEmoji &&
            !divEmoji.contains(event.target) &&
            event.target !== document.getElementById("openEmoji")
        ) {
            toggleModal(divEmoji, "hidden", true);
            divEmoji.innerHTML = "";
        }
        ...
        fetch("/api/render-component-post-show?user_id=" + userId)
            ...
            .then((html) => {
                ...
                if (description) {
                    ...
                    let descriptionPost =
                        document.getElementById("descriptionPost");
        ...
        fetch("/api/crop-image", {
            ...
            .then((data) => {
                ...
                img.onload = function () {
                    ...
                    document
                        .getElementById("store-post")
                        .appendChild(inputPhoto);
    ...
    function formatTime(secondsElapsed) {
        ...
        if (secondsElapsed < secondsInMinute) {
            return `hace ${secondsElapsed} segundo${
                secondsElapsed > 1 ? "s" : ""
            }`;
